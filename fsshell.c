...
int displayFiles (fdDir * dirp, int flall, int fllong)
	...
	while (di != NULL) 
		...
		if ((di->d_name[0] != '.') || (flall)) //if not all and starts with '.' it is hidden
			...
			if (fllong)
				{
				char * abs_path = combine_paths(dirp->path, di->d_name);
				fs_stat (abs_path, &statbuf);
                format_time(statbuf.st_modtime, time_s, 256);
				printf ("%s    %9ld   %s   %s\n", fs_isDir(abs_path)?"D":"-",
                        statbuf.st_size, time_s, di->d_name);
				free(abs_path);
				}
			else
				{
...
int cmd_cp (int argcnt, char *argvec[])
    ...
    if (argcnt == 2) {
        printf("Missing required destination!\n");
        return (0);
    }
    ...
    if (fs_isDir(dest))
        {
            // if so get filename from source and append to destination
            const char * filename = fs_getFilename(src);
            newdest = fs_appendFilename(dest, filename);
            dest = newdest;
        }
...
int cmd_mv (int argcnt, char *argvec[])
    ...
    {
        int result = fs_mv (argvec[1],argvec[2]);
        if (result == -1)
        {
